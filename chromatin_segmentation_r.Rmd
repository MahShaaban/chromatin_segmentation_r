...
- *ChromHmm* is a Java program to learn chromatin states from multiple sets of histone markers ChIP-seq data
...
- We then used `Gviz`'s `AnnotationTrack` to render the ranges as tracks grouped by
...
---
```{r plot_track, fig.align='center',fig.cap="Segmentation track"}
# plot the track
...
```
---
```{r tracks, echo=FALSE, fig.align='center', fig.width=4, fig.cap='Full tracks'}
# put all tracks together
plotTracks(
    list(
        itrack,
...
# compare models plots
par(mfrow=c(1,2))
h1 <- compare_models(objs,
               plot = TRUE,
               xlab = 'Model',
               ylab = 'State Correlation')

h2 <- compare_models(objs, type = 'likelihood',
               plot = TRUE,
               xlab = 'Model',
               ylab = 'Model Likelihood')
h1 + h2
